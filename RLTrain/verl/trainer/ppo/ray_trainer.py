...
def recall_ret(sorted_docs, golden_answer_list):
    ...
    Args:
        sorted_docs: Ranked retrieved documents.
        golden_answer_list: Relevant documents.

    Returns:
        Recall value.
    """
    # Convert both lists to sets to speed up intersection lookup.
    ...
    # Return zero if there are no relevant documents.
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        attention_mask = data.batch['attention_mask']
        response_mask = attention_mask[:, -response_length:]
        token_level_rewards = data.batch['token_level_rewards'][:, 0, :]
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    extra_info_list = []
                    if 'file_name' in batch.non_tensor_batch:
                        ...
                        for reference_page, extra_info in zip(batch.non_tensor_batch['reference_page'], batch.non_tensor_batch['extra_info']):
                            extra_info_list.append(dict(reference_page=reference_page, question=extra_info['question'], answer=extra_info['answer']))
                ...
                # collect metrics
                train_metrics = compute_data_metrics(batch=batch, use_critic=self.use_critic)
                train_score_overall = train_metrics[f'critic/score/mean']
                if train_score_overall > best_score:
                    self._save_checkpoint()
                    best_score = train_score_overall
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            with _timer('step', timing_raw):
                ...
                ndcg_tensor_lst.append(ndcg_tensor)
                recall_tensor_lst.append(recall_tensor)
                
                data_source_lst.append(test_batch.non_tensor_batch.get('data_source', ['unknown'] * reward_tensor.shape[0]))

        reward_tensor = torch.cat([rw.sum(-1) for rw in reward_tensor_lst], dim=0).cpu()  # (batch_size,)
        # ndcg and recall are lists, not tensors
        ndcg_list = [item for sublist in ndcg_tensor_lst for item in sublist]
        recall_list = [item for sublist in recall_tensor_lst for item in sublist]
        ...
        for i in range(len(ndcg_list)):
            data_source = data_sources[i]
            if data_source not in data_source_ndcgs:
                data_source_ndcgs[data_source] = []
            data_source_ndcgs[data_source].append(ndcg_list[i])

        for data_source, ndcgs in data_source_ndcgs.items():
            if len(ndcgs) > 0:
                metric_dict[f'val/test_ndcg_score/{data_source}'] = np.mean(ndcgs)
            
        
        data_source_recalls = {}
        for i in range(len(recall_list)):
            data_source = data_sources[i]
            if data_source not in data_source_recalls:
                data_source_recalls[data_source] = []
            data_source_recalls[data_source].append(recall_list[i])
        for data_source, recalls in data_source_recalls.items():
            if len(recalls) > 0:    
                metric_dict[f'val/test_recall_score/{data_source}'] = np.mean(recalls)
        ...
        overall_ndcgs = []
        for i in range(len(ndcg_list)):
            overall_ndcgs.append(ndcg_list[i])
        if len(overall_ndcgs) > 0:
            metric_dict[f'val/test_ndcg_score/overall'] = np.mean(overall_ndcgs)
        
        overall_recalls = []
        for i in range(len(recall_list)):
            overall_recalls.append(recall_list[i])
        if len(overall_recalls) > 0:
            metric_dict[f'val/test_recall_score/overall'] = np.mean(overall_recalls)
